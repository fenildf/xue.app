...
function changeVerificationImg(imgId) {
    var newVerificationImg = app.url + '/Verifications/show?' + generateMixed(12);
...
var jsArray = {};
xue.loadScript = function(scriptName, callback) {
...
xue.placeholder = function(dom) {
    var box = $(dom);
    box.each(function() {
        var that = $(this),
            old = that.attr('placeholder');
        that.on('focus', function() {
            that.data('default', old);
            that.attr('placeholder', '');
        });
        that.on('blur', function() {
            that.attr('placeholder', old);
        });
    });

};
/**
 * 格式化日期：将字符串转换为指定格式的日期
 * @type {[type]}
 * //test
 ...
 */
xue.date = function(format, timestamp) {
 ...
 */
xue.removeHTMLTag = function(str){
...
xue.CovertFromXmlTag = function(value) {
...
xue.json.parse = function(jsonString) {
	var newjson = null;
    if (window.JSON) {
        newjson = window.JSON.parse(jsonString);
    }else{
    	newjson = (new Function('return '+ jsonString +';'))();
    }
    return newjson;
    ...
    if (xml.hasChildNodes()) {
        for(var i = 0; i < xml.childNodes.length; i++) {
            ...
            if (typeof(obj[nodeName]) == "undefined") {
                obj[nodeName] = xue.xmlToJson(item);
            } else {
                ...
                obj[nodeName].push(xue.xmlToJson(item));
            }
        }
    }
    return obj;
